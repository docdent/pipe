...
static uint8_t adcNr;
volatile KeyInfo adcKeys[1];
uint8_t keyWants[6];
...
# 68 ".././hwtimer.c"
 ...
 for (uint8_t i = 0; i < 6; i++){
  ...
  if (i+1 == 6){
   keyWants[i] = 0x02 | 0x04;
  } else {
   keyWants[i] = 0x02 | 0x08;
  }
 }
}
...
uint32_t test_PipeModule(uint8_t moduleNr){
 uint32_t result;
 if (moduleNr >= 8) {
...
static inline void timerTimers(){
 ...
 uint8_t* pTime = (uint8_t *) &(time_Uptime[0]);
 if (++(*pTime) == (1000 / 4)){
...
# 382 ".././hwtimer.c"
  ...
  if (absDifference(oldADC,newADC) < 4) {
   ...
   switch (adcNr) {
    ...
    if (newADC < (23 + 45)/2) {
     newKey = 4;
    } else if (newADC < (45 + 81)/2) {
     newKey = 3;
    } else if (newADC < (81 + 128)/2) {
     newKey = 5;
    } else if (newADC < (128 + 173)/2) {
     newKey = 1;
    } else if (newADC < (173 + 210)/2) {
     newKey = 2;
    } else if (newADC < (210 + 0xFF)/2) {
     newKey = 6;
    } else {
     newKey = 0xFF;
    }
   ...
   if (oldKey != newKey){
    ...
    if (oldKey == 0xFF) {
     ...
     adcKeys[adcNr].keyRepeating = 0;
     if (keyWants[newKey-1] & 0x01) {
      message_push(0x00 | newKey);
     }
    } else {

     if (keyWants[oldKey-1] & 0x02) {
      if ((adcKeys[adcNr].keyRepeating & (1 | 2)) == 0){

       message_push(0x00 | oldKey);
      }
     }
    }
   } else {
    ...
    if (newKey != 0xFF) {
     ...
     if (keyWants[oldKey-1] & 0x08) {
      ...
      } else if ((adcKeys[adcNr].keyRepeating == 1) && (adcKeys[adcNr].keyTimer == (200 / (4 * 4)))) {
...
void softKey_WantLong(uint8_t wantLong){
 ...
 for (uint8_t i = 0; i < 6; i++){
  ...
  if ((i+1 == 2) || (i+1 == 3) || (i+1 == 4) || (i+1 == 1)) {
   keyWants[i] = 0x02 | (wantLong == 0 ? 0x08 : 0x04);
  }
 }
...
# 515 ".././hwtimer.c"
  ...
  *pInByte = newPipeStat;
  ...
  if ((statChange)!= 0) {
   ...
   myMessage.message8[0] = statChange;
   myMessage.message8[1] = 0x20 | shiftBitNr;
   pipeMsgPush(myMessage);
  }
  statChange = (~newPipeStat & oldPipeStat) & local_pipe_ModuleAssnRead;
  if ((statChange)!= 0) {
...
# 586 ".././hwtimer.c"
 ...
 switch (++msecCtr & 0x03) {
  ...
  case 0: timerADC(); break;
  case 1: if (pipeProcessing != 0x80) {
    timerPipeIO();
   }
   break;
  case 2: timerTimers(); break;
  case 3: if (pipeProcessing != 0x80) {
    timerPipeProcess();
   }
   break;
 }
