...
volatile uint8_t midiRxBuffUsage;
volatile uint8_t midiTxBuffUsage;
...
uint8_t serUSBRxBuffer[64];
uint8_t serUSBTxBuffer[2048];
...
uint8_t serESPRxBuffer[128];
uint8_t serESPTxBuffer[512];
uint8_t serESPInBuffer[8];
uint8_t serESPMidiTmp[3];
...
void serial1MIDISend(uint8_t data){
 ...
 if ((midi_Setting.VelZero4Off) && (data >= 0x80)){
...
# 100 ".././serial.c"
 ...
 uint8_t newIndex = (midiTxInIndex+1) & 0xFF;
 ...
 uint8_t txBufferLoad = (midiTxInIndex > midiTxOutIndex ? midiTxInIndex - midiTxOutIndex : 256 - midiTxOutIndex + midiTxInIndex);
...
uint8_t serial1MIDIReadRx(){
 ...
 if (midiRxInIndex == midiRxOutIndex) {
  ...
  serial0USB_logMIDIin(result);
  midiRxOutIndex = (midiRxOutIndex+1) & 0xFF;
 }
 return result;
}
...
# 142 ".././serial.c"
 ...
 uint8_t newIndex = (midiRxInIndex+1) & 0xFF;
 ...
 uint8_t rxBufferLoad = (midiRxInIndex > midiRxOutIndex ? midiRxInIndex - midiRxOutIndex : 256 - midiRxOutIndex + midiRxInIndex);
...
{
 uint8_t count = 80;
 while (((*s) != 0) && (! (serUSBOvflFlag == 0xFF)) && (count != 0)){
...
uint8_t serial0SER_USBReadRx(){
 uint8_t result;
 uint8_t* index = (uint8_t*) serUSBRxOutIndex;
...
void serial3SER_ESP_sendString(char *s){
 uint8_t count = 255;
 char c;
 while (((c= *s++) != 0) && (! (serESPOvflFlag == 0xFF)) && (count-- != 0)){
  serial3SER_ESPSend(c);
 }
}
...
uint8_t serial3SER_ESPReadRx(){
 uint8_t result;
 uint8_t* index = (uint8_t*) serESPRxOutIndex;
...
# 465 ".././serial.c"
 ...
 if (index == outIndex) {
  ...
  serESPRxInIndex = index;
 }

 uint8_t* inpBufferPtr = & serESPInBuffer[8 -1];

 for (uint8_t i = 0; i < 8 -1; i++){
  *(inpBufferPtr) = *(inpBufferPtr-1);
  inpBufferPtr--;
 }
 *inpBufferPtr = receivedByte;
}
