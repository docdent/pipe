...
PipeMessage_t pipeMessages [256];
...
void message_push (uint8_t msg){
 ...
 uint8_t newMsgInIndex = (msgInIndex+1) & 0x1F;
...
# 60 ".././message.c"
  ...
  if (msgOutIndex == msgInIndex) {
   ...
   msgOutIndex = (msgOutIndex+1) & 0x1F;
   result = mess;
  }
 }
 return result;
...
void pipeMsgPush (PipeMessage_t pipeNr) {
 if (msgPipe_Handling){
  ...
  if (pipeMsgInIndex >= 256) {
   pipeMsgInIndex = 0;
  }
  if (pipeMsgInIndex == pipeMsgOutIndex) {

   pipeMsgInIndex--;
   if (pipeMsgInIndex == 0xFF) {
    pipeMsgInIndex = 256 -1;
   }
   msgPipeOverflow = 0xFF;
  }
 }
}
